...
def generage_pages_recursive(dir_path_content, template_path):
    full_path = ""
    if(dir_path_content == ""):
        full_path = "content/"
    else:
    ...
    for file in files:
        ...
        dest_full_path = full_path.replace("content", "public")
        ...
        if(os.path.isfile(full_path + file)):
            ...
            print(f"running command: generate_page({full_file_path}, {template_path}, {dest_full_file_path})")
            generate_page(full_file_path, template_path, dest_full_file_path)
        else:
            ...
            if(os.path.exists(dest_full_path)):
                ...
                print(f"directory: {dest_full_path} does not exist, creating it")
                os.mkdir(dest_full_path)
            print(f"running command: generage_pages_recursive({full_path + file + "/"}, {template_path})")
            generage_pages_recursive(full_path + file + "/", template_path)
    return
...
def main():
    clean_public()
    copy_static()
    generage_pages_recursive("", "template.html")
